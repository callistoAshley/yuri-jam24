...
// TODO: would be nice to extract this function into a separate file of helper
// functions
...
{
    EventLoader *interpreter = calloc(1, sizeof(EventLoader));
    if (!interpreter)
        return NULL;
...
{
    for (LinkedListNode *node = interpreter->events->first; node;
         node = node->next)
    {
