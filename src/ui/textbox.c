...
#define INPUT_BUTTONS_DOWN(resources) (input_is_pressed(resources->input, Button_Jump) || input_is_pressed(resources->input, Button_Cancel))
...
void textbox_init(Textbox *textbox, Resources *resources)
{
    ...
    {
        TextureEntry *texture = texture_manager_load(&resources->graphics->texture_manager, TEXTURE_PATH("textbox.png"), &resources->graphics->wgpu);
        WGPUTexture wgpu_texture = texture_manager_get_texture(&resources->graphics->texture_manager, texture);

        Quad quad =
        {
            .rect = rect_from_size((vec2s)
            {
                .x = wgpuTextureGetWidth(wgpu_texture),
                .y = wgpuTextureGetHeight(wgpu_texture)
            }),
            .tex_coords = RECT_UNIT_TEX_COORDS,
        };
        QuadEntry quad_entry = quad_manager_add(&resources->graphics->quad_manager, quad);

        Transform transform = transform_from_scale(VEC3_SPLAT(WINDOW_SCALE / 2));
        TransformEntry transform_entry = transform_manager_add(&resources->graphics->transform_manager, transform); 

        ui_sprite_init(&textbox->sprite, texture, transform_entry, quad_entry, 0.0f);
        textbox->sprite_entry = layer_add(&resources->graphics->ui_layers.middle, &textbox->sprite);
    }
}
...
static void update_text(Textbox *textbox, Resources *resources)
{
    ...
    font_render_text_to(
        ...
        texture_manager_get_texture(&resources->graphics->texture_manager, textbox->text_sprite.texture), 
        text, 
        color, 
...
static void process_escape_code(Textbox *textbox, Resources *resources)
{
    ...
    if (textbox->text[textbox->text_idx] == '\\')
    {
        ...
        memmove(textbox->text + textbox->text_idx, textbox->text + textbox->text_idx + 2, strlen(textbox->text) - textbox->text_idx); // teehee
    }
}
...
static void remove_text(Textbox *textbox, Resources *resources)
{
    ...
    layer_remove(&resources->graphics->ui_layers.foreground, textbox->text_sprite_entry);
    memset(textbox->text, 0, sizeof(textbox->text));
}
...
void textbox_display_text(Textbox *textbox, Resources *resources, char *text)
{
    ...
    WGPUTexture texture = font_render_text(&resources->fonts->monogram.medium, textbox->text, color, &resources->graphics->wgpu);

    u32 width = wgpuTextureGetWidth(texture), height = wgpuTextureGetHeight(texture);
    ...
    ui_sprite_init(
        &textbox->text_sprite, 
        texture_manager_register(&resources->graphics->texture_manager, texture, "textbox_text_sprite"),
        transform_entry,
        quad_entry, 
        1.0f
    );
    textbox->text_sprite_entry = layer_add(&resources->graphics->ui_layers.foreground, &textbox->text_sprite);
}
