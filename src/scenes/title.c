...
void title_scene_init(Resources *resources, void *extra_args)
{
    (void)extra_args;

    TitleScene *title_scene = malloc(sizeof(TitleScene));
    resources->scene = (Scene *)title_scene;
    ...
    {
        WGPUTexture texture =
            font_render_text(&resources->fonts.compaq.medium, options[i], color,
